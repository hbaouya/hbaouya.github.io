...
const Projects: React.FC = () => {
  ...
  const management = profile.managementRole;
    ...
    <section className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 mb-8">
        ...
        {projects.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12">
              {projects.map((proj) => (
                  <div key={proj.id} className="flex flex-col bg-slate-50/50 rounded-2xl border border-slate-200 p-6 hover:shadow-lg hover:shadow-slate-200/50 hover:border-blue-200 transition-all duration-300 group shadow-sm">
                      <div className="flex justify-between items-start mb-4">
                          <div>
                              <h4 className="font-bold text-slate-900 text-lg group-hover:text-primary transition-colors leading-tight">{proj.name}</h4>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-200/60 mt-auto">
                          ...
                          {proj.url && (
                              <a href={proj.url} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-primary hover:text-blue-700 flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-blue-55 transition-colors">
          ...
          <p className="text-slate-400 italic text-center py-8">No projects compiled yet.</p>
        )}
           ...
           <>
             ...
             <div className="relative overflow-hidden p-6 rounded-2xl border border-blue-100 bg-gradient-to-br from-white to-blue-50 shadow-sm group">
                 ...
                 <div className="relative flex items-start gap-5">
                     ...
                     <div>
                        <h4 className="font-bold text-slate-900 text-xl mb-1">{management.title}</h4>
                        <p className="text-slate-700 font-medium">{management.institution}</p>
                        <div className="mt-3 flex flex-wrap gap-3">
                            <span className="text-xs font-bold text-blue-600 bg-blue-100 px-2 py-1 rounded uppercase tracking-wide">{management.period}</span>
                        </div>

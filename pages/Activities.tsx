...
const Activities: React.FC = () => {
  ...
  const reviewersList = activities.reviewers || [];
                      ...
                      {activities.items.map((item, idx) => (
                          <li key={idx} className="flex items-start group">
                              <span className="flex-shrink-0 w-10 h-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center mr-4 group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
                                  <i className={`${item.icon || 'fas fa-gavel'}`}></i>
                              </span>
                              <div className="pt-1">
                                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                                      <span className="font-bold text-slate-800 block text-base leading-tight">{item.role}</span>
                                      {item.url && (
                                  ...
                                  {Array.isArray(item.details) ? (
                                      <ul className="space-y-1">
                                          {item.details.map((detail, dIdx) => (
                                              <li key={dIdx} className="text-sm text-slate-600 flex items-start leading-relaxed">
                                      ...
                                      <span className="text-sm text-slate-600 leading-relaxed">{item.details}</span>
                                  )}
                              </div>
                          </li>
                      ))}
            ...
            {reviewersList.length > 0 && (
              <div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                      {reviewersList.map((activity, idx) => (
                          <div key={idx} className={`p-5 bg-slate-50 rounded-xl border border-slate-100 hover:shadow-md transition-shadow ${idx === reviewersList.length - 1 ? 'md:col-span-2' : ''}`}>
                              <h4 className="font-bold text-slate-800 mb-3 flex items-center">
                                  <i className={`${activity.icon || 'fas fa-book-open'} text-primary mr-2`}></i> {activity.publisher}
                              </h4>
                              <ul className="space-y-1">
                                  {activity.journals.map((journal, jIdx) => (
                                      <li key={jIdx} className="text-sm text-slate-600 flex items-start pl-2 border-l-2 border-slate-200 hover:border-primary transition-colors py-0.5 leading-snug">
                                          {journal}
                                      </li>
                                  ))}
                              </ul>

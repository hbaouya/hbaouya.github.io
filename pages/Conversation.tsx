...
const Conversation: React.FC = () => {
  ...
  const selectedInterview = interviewsList.find(i => i.id === selectedId);
    ...
    return (
        <section className="bg-white border-t-4 border-black py-8 mb-8 font-serif text-slate-900 animate-in fade-in duration-500">
            ...
            <div className="border-b-2 border-black pb-6 mb-8 flex flex-col md:flex-row justify-between items-end px-4 sm:px-6">
                <div className="mb-4 md:mb-0 max-w-3xl font-serif">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 px-4 sm:px-6">
                ...
                <div className="mb-10">
                    ...
                    {selectedInterview.questions && selectedInterview.questions.length > 0 && (
                      <div className="prose prose-slate prose-lg max-w-none font-serif text-slate-800">
                          ...
                          {selectedInterview.questions.map((q, qIdx) => (
                            <React.Fragment key={qIdx}>
                              <h4 className="font-bold text-xl text-black mb-3 mt-8">{q.question}</h4>
                ...
                {/* Sidebar Column */}
                <div className="lg:col-span-4 lg:border-l lg:border-slate-200 lg:pl-8 font-sans receipts sticky top-8 h-fit">
                    <h4 className="font-bold text-xs uppercase tracking-widest mb-6 border-b border-slate-200 pb-2">At a Glance</h4>
                    
                    {selectedInterview.at_a_glance && (
                      <ul className="space-y-8">
                          {selectedInterview.at_a_glance.map((glance, gIdx) => (
                            <li key={gIdx}>
                                <span className="block text-[10px] text-slate-400 uppercase tracking-wider mb-1">{glance.label}</span>
                                <span className="block font-serif text-xl leading-snug text-slate-900">{glance.value}</span>
                            </li>
                          ))}
                      </ul>
                    )}
                    
                    <div className="mt-10 p-6 bg-slate-5/50 border border-slate-100 rounded-sm">
                        <h5 className="font-bold text-xs uppercase mb-3 text-slate-800">About the Team</h5>
         ...
         {interviewsList.length > 0 ? (
           <div className="grid grid-cols-1 gap-6">
               {interviewsList.map((interview) => {
                   ...
                   const thumbClass = interview.image ? "object-cover grayscale group-hover:grayscale-0" : "object-contain p-4 b-white";
                   return (
                       <div 
                           key={interview.id} 
                       ...
                       >
                           <div className="flex flex-col md:flex-row gap-6 items-start">
                               <div className="w-full md:w-48 h-32 flex-shrink-0 bg-slate-100 rounded-lg overflow-hidden border border-slate-100">
           ...
           <p className="text-slate-400 italic text-center py-8">No conversation items available.</p>
         )}
    </section>
  );
};
